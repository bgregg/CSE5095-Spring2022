...
% SF_ELEVATOR_GUI  Interface module to Stateflow Elevator Control Logic
% example
...
case 'init'
    if nargin < 3
        error('To initialize UI you need to specify the model name and the elevator car IDs');
    end

    modelName = varargin{1};
    allCarIds = varargin{2};

    % Initialize the elevator example GUI figure
    ...
    if nargin == 3
        ...
        floorA = str2double(varargin{3}(1));
        floorB = str2double(varargin{3}(2));
        
        if floor(floorA) ~= floorA || floorA < 1 || floorA > numFloor
             error('Incorrect initial floor number for elevator A : %d', floorA)
        elseif  floor(floorB) ~= floorB || floorB < 1 || floorB > numFloor
             error('Incorrect initial floor number for elevator B : %d', floorB)
        end;
        
        carAPos = floorHeight * (floorA - 1);
        carBPos = floorHeight * (floorB - 1);
    end;
    ...
    carInternalViewWidth = 80; %80;
    carInternalViewHeight = 100;
    ...
    btnSize = 10;
    baseX = carAaxes-carInternalViewWidth/2+2;
    baseY = 7;
    
    for f = 1:numFloor
        btnX = 1.75*mod(f-1,3)*btnSize+baseX+5;
        btnY = 2*floor((numFloor-f)/3)*btnSize+baseY+5;
    ...
    fireBtnA = text('Parent',axeCars, ...
                    'Position',[carAaxes+carInternalViewWidth/2-8 45], ...
                    'String','FIRE', ...
    ...
    baseX = carBaxes-carInternalViewWidth/2+2;
    ...
    fireBtnB = text('Parent',axeCars, ...
                    'Position',[carBaxes+carInternalViewWidth/2-8 45], ...
                    'String','FIRE', ...
                    'BackgroundColor',inactiveFireColor, ...
                    'Color','w', ...
                    'FontSize',7, ...
